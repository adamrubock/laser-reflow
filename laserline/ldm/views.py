        ...
        if run_request is not None:
            if run_request == 'DO_NOTHING':
                ...
                if running:
                    ...
                    ok_to_start = (r.getbit('digital_inputs', 2) or
                                   r.getbit('digital_inputs', 3) or not
                                   r.getbit('digital_inputs', 4) or not
                                   r.getbit('digital_inputs', 5) or not
                                   r.getbit('digital_inputs', 6))
                    if ok_to_start:
...
def info(request):
    r = Redis(password='laserr3flow')
    response = {name: float(val)
                for name, val in r.hgetall('analog_inputs').items()}

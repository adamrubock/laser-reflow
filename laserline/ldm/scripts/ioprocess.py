...
class LaserlineIO(object):
    def __init__(self):
        ...
        self.power_dac = adafruit_mcp4725.MCP4725(self.tca[7], address=0x60)
        ...
        # MCP3248 adc is read with SMBus because
        # there's no decent library for it. addresses 0x68 and 0x6e

        self.digital_in_1 = pcf8574.PCF8574(1, 0x60)
        self.digital_in_2 = pcf8574.PCF8574(1, 0x61)
        self.digital_out = pcf8574.PCF8574(1, 0x22)
        ...
        pipe.set('digital_inputs', '\x00')
        pipe.set('digital_outputs', '\x00')
            ...
            with self.i2c_lock:
                ...
                if self.run_active.is_set():
                    ...
                    if self.r.delete('start_run'):  # delete returns 1 if key existed
                        ...
                        self.run_toggle.notify()
                toggle_happened = self.run_toggle.wait(timeout=.1)
        ...
        while True:
            with self.i2c_lock:
                if self.run_toggle.wait():  # using "if" as a spurious wakeup defense mechanism
                    ...
                    logging.info('x ' + self.r.get('x_axis') +
                                 ' y ' + self.r.get('y_axis'))
                    ...
                    for duration, level in zip(durations, levels):
                        try:
                            ...
                            logging.info('actually waited milliseconds: ' +
                                         str((currtime - prevtime) / timedelta(milliseconds=1)))
                            prevtime = currtime
                            logging.info('power set to: ' + str(level))
                            logging.info(
                                'planning to wait ms:' + str(duration))
                        except OSError:
                            logging.error(
    ...
    def update_outputs(self):
        ...
        try:
            ...
            output_list.extend([True])  # laser power off
